# Casper Discord Bot — Codex Skill Guide

This file provides Codex (the coding agent) with the context, conventions, and patterns needed when working on this project.

## Project Overview

**Casper Discord Bot** is a Discord bot built with TypeScript and `discord.js` v14 that provides blockchain query and notification features for the Casper network. It uses a modular **Skill-based architecture** where each feature is an independent skill module managed by a central `SkillManager`.
...
```
src/
├── index.ts                 # Entry point — Discord client init, skill registration
├── core/
...
│   └── push/                # Message push to Discord channels skill
├── services/
...
2. **Define commands** (`commands.ts`):
   - Use `new SlashCommandBuilder()` and export the builder instance(s).
   - Use `.setName(kebab-case)` for command names.

3. **Implement handler** (`handler.ts`):
   - Export async functions that accept `ChatInputCommandInteraction`.
   - Always call `await interaction.deferReply()` at the start (the SkillManager does not defer for you).
...
4. **Create skill class** (`<Name>Skill.ts`):
   ...
   - Implement `handleCommand(interaction)` to route to the correct handler.
...
- Use `EmbedBuilder` for rich responses (set color, fields, timestamp, footer).
